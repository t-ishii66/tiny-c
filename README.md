...
コンパイラのソースコードは約1500行で、C 言語で書かれている。また　C言語の知識だけでtiny-cが理解できるようにドキュメントを構成した。
...
| [章6: ポインタと配列](docs/ch06/00_overview.md) | `char *s = "hello"; printf("%s\n", s);`、ポインタ・配列・グローバル変数・ブロックスコープ | lvalue/rvalue の二面性、`gen_addr` 関数、`&` と `*` の対称性、append-only リスト切り戻しによるスコープ管理 |
| [章7: 最適化とバックパッチ](docs/ch07/00_overview.md) | (言語機能の追加なし) | AST 最適化（定数畳み込み + 代数的単純化）、バックパッチで Phase 1 を消して codegen を単一パスに、ピープホール最適化 |
