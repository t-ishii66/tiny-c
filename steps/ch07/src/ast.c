...
int type_size(Type *t) {
    ...
    if (t->is_array) return t->base_size * t->array_size;
    return t->base_size;
}
...
static void print_type(Type *t) {
    if (!t) { printf("?"); return; }
    if (t->base_size == 1) printf("char");
    else printf("int");
    if (t->is_pointer) printf("*");
    if (t->is_array) printf("[%d]", t->array_size);
}
...
void print_ast(Node *node, int level) {
    ...
    case NODE_ASSIGN:
        ...
        break;
    case NODE_VAR_DECL:
        printf("VAR_DECL %s : ", node->name);
        print_type(node->type);
        printf("\n");
        if (node->expr) print_ast(node->expr, level + 1);
        break;
    case NODE_GLOBAL_VAR_DECL:
        printf("GLOBAL_VAR_DECL %s : ", node->name);
        print_type(node->type);
        printf("\n");
        break;
    case NODE_INDEX:
    ...
    case NODE_FUNC_DEF:
        ...
        for (NodeList *l = node->params; l; l = l->next) {
            indent(level + 1);
            printf("PARAM %s : ", l->node->name);
            print_type(l->node->type);
            printf("\n");
        }
        print_ast(node->body, level + 1);
        break;
