...
ch04 では `{ ... }` をブロック文として扱うが、**この章ではまだブロックスコープを実装していない**。codegen は ch04 時点では関数全体で 1 つのフラットなシンボルテーブルなので、別ブロックで同名の `int y = ...;` を書くと「再宣言」エラーになる。**ch07 でスコープを導入** するとき、`locals` 連結リストの head をスコープ進入時に保存し、抜けるとき切り戻す仕組みに拡張する（`{ int x = 1; } { int x = 2; }` が通るようになる）。
