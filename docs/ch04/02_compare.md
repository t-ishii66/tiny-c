...
これらの命令の **書き込み先（オペランド）は 8 ビットレジスタに限られる**。32ビット `%eax` を直接指定することはできず、その下位 8 ビットを指す `%al` を使う:
...
逆順 (`cmpl %eax, %ecx`) で書くとフラグの符号が逆向きになり、すべての比較が裏返る。**ch02 で確立した順序（rhs → ecx, lhs → eax）を維持するから、比較も自然に書ける**。
