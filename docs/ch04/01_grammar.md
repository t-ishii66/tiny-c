...
階層を深くするほど優先順位が高い、左再帰で左結合、というパターンは ch02 から変わらない。
...
ch04 では `{ ... }` をブロック文として扱うが、**ブロックスコープは実装していない**。tiny-c の codegen はシンボルテーブルがフラット（関数全体で1つ）なので、別ブロックで同名の `int y = ...;` を書くと「再宣言」エラーになる。スコープ管理は意図的に省略した。

## 次へ

次のサブ章 02（`02_compare.md`）では、比較演算と論理否定の codegen を見る。
