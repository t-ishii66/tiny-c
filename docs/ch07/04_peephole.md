...
## 1. パイプラインの中の位置（memstream の復習）

`open_memstream` でアセンブリ出力をメモリバッファに溜める仕組みは、ch07/02 でバックパッチのために導入済み。ピープホールはこの **同じバッファ** を再利用する ── codegen が書き終わった asm を、stdout に流す前に一度走査して書き換える後処理として乗る。
...
codegen(program, mem);     /* バックパッチもここで完了している */
fclose(mem);
peephole(buf, (int)len, stdout);   /* この節で追加: 行単位の書き換え */
```

codegen 自体は **無変更**。バッファの中身を読んで書き換える peephole 関数を、codegen と stdout の間に挟むだけ。
...
`ret` の次の `jmp .Lendif_0` は到達不能（直前で関数を抜けるので）。デッド状態のまま命令行として消える。次の `.Lelse_0:` ラベルでデッド状態が解除される ── `ret` の直線的な制御フローでは到達できなくても、**ラベルは別の場所からの `jmp` / `je` のジャンプ先になりうる**ので、ラベル以降のコードは「届く可能性がある」とみなして残す必要がある。これで `else` 節のコードはちゃんと残る。**「途中 ret」も「末尾 ret」も同じルールで処理される**。
...
最後の節（`05_build.md`）で、`optimize.c` 全体の差分と `main.c` の変更を並べ、ビフォー・アフターのデモを示す。
