...
`INT '*'` のように `*` をトークンとして使っているのに注意。`*` は **二項演算子**（乗算）でもあるし **単項演算子**（間接参照）でもある。bison は文脈で区別する ── ここでは「`INT` の直後の `*`」なので別意味に解釈されることはない。
...
`a[0]` も `a[i+1]` も同じ規則で扱える（インデックスは任意の式）。tiny-c では実用上 `IDENT '[' expr ']'` の形だけサポート（`(p+1)[i]` のような複雑な左辺は許さない）── 文法を簡素にするため。
...
`*` は **二項** で `mul_expr '*' unary`、**単項** で `'*' unary` の両方に登場するが、bison は文脈で区別する。`a * b` は `mul_expr` レベルでマッチ、`*p` は `unary` レベルでマッチ。
...
`s` の型が `char*`、`STRING_LIT` で `"hello"` を保持、`s[0]` が `INDEX` ノード。codegen ではこの型情報を見て、char 単位で 1 バイトずつ読み出すコードを生成する（次のサブ章）。

## 12. 次へ

文法と AST に型が入った。次のサブ章（`02_lvalue.md`）で、コード生成側の核心 ── **`gen_addr` 関数** と **lvalue / rvalue の対称性** ── を見る。`*` と `&` がコード生成の上でどう対応関係にあるか、配列名がなぜ自動的にアドレスになる（"decay"）か、すべて `gen_addr` と `gen_expr` の使い分けで説明できる。
