...
`gen_stmt` から呼ばれるのは常に `gen_expr` で、`gen_addr` が直接呼ばれることはない。`gen_addr` は上の 2 場面に該当したとき、`gen_expr` の中からサブルーチンとして呼ばれる。
...
`default` ケースに来ると「lvalue でないノードのアドレスを取ろうとした」エラー。`42`（リテラル）や `f()`（関数呼び出し）は lvalue でないので、そのアドレスを取ろうとすればここで弾かれる。
...
`int x` も `int *p` も `int a[5]` も、IDENT としては **同じ 1 命令** で扱う ── 変数の格納場所の先頭アドレスを `leaq` で取るだけ。
...
つまり「変数の場所のアドレスを取る」という IDENT の役割は型に関わらず同じで、型の違いは別の場面（add_local でのバイト数、load/store のサイズ、添字でのスケーリング）に現れる。
...
ここがポインタの真髄: `*p` の **アドレス** が欲しいなら、`p` の **値** を取ればいい。なぜなら p の値そのものがそのアドレスだから（節 3 の対応表で「`*p` の lvalue は `p` の rvalue と一致」と書いた通り）。
...
そのうえで `(base) + i × elem_size` を計算してアドレスを `%rax` に置く。配列とポインタを **統一的に** 扱えるのがこの設計の効果。
...
`x = 5;`、`p = some_addr;`、`*p = 10;`、`a[i] = 7;` ── すべて同じパターン。`gen_addr` が左辺の形を吸収してくれるので、ASSIGN の codegen は分岐なし。ch03 では `if (lhs->kind != NODE_IDENT) error` という特別判定があったが、もう要らない。**lvalue かどうかは `gen_addr` が「not an lvalue」エラーを出す形に集約される**。
...
`gen_expr` の中で `gen_addr` を呼ぶだけ。普段の識別子読み出し（`gen_expr(x)`、節 6.1）なら `gen_addr` で得たアドレスからさらに値を load する（= `%rax` に入っているアドレスを参照して、そのアドレスが指すメモリの内容を読み出し、`%rax` に上書きする）が、`&x` ではその load をスキップする。結果として **`%rax` にはアドレスがそのまま残り、そのアドレスが指す先のデータには触らない**。つまり **`&` は load を取り去る** といえる。
