...
- **「`program` は `stmt` で構成される」** ── プログラムというのは、つまるところ文 (statement) ひとつだ、と言っている。
...
`:` の左にあるのが「定義する名前」、右にあるのが「その名前の中身（並び）」。「**:** は『〜とは』、`;` は文法ルールの終わり」と読めばよい。
...
ここが bison を使うことのキモだ。**ルールが実際にマッチしたとき（= 入力のトークン列がそのルールに当てはまったとき）、対応するアクション中の C コードが実行される**。たとえば上の規則で言えば、「`RETURN`、`expr`、`;`」という並びが認識された瞬間に、`{ ... }` の中身が動く。

文法ルールは「形を定義するもの」、アクションは「形が見つかったときに何をするか」── この2つがセットになっている。bison は形のチェックを自動でやってくれて、我々は **「形ができたときに何をするか」だけ** を書けばよい。今回の場合はそこで AST のノードを作って繋いでいく。
...
**左辺の非終端記号（`expr` や `ret_stmt` など）はそれ自身が「値」を持つ**。それを書き込む先が **`$$`** だ。アクションで `$$ = ...` と決めた値は、上位のルールがこの非終端記号を参照するとき（`$2` などとして）に取り出される。
...
これは「`expr` の値は `INT_LIT` の値そのもの」という意味になる。`$1` は右辺1番目（`INT_LIT`）の値、`$$` は左辺（`expr`）の値。「`INT_LIT` の値をそのまま `expr` の値にする」と言っている。
...
`$2` は expr ルールが返したノード。それを `new_return` で包んで、return 文ノードを作る。
...
次のサブ章（`04_ast.md`）では、ここで作った木そのものの実装（`ast.h` と `ast.c`）を設計の意図から見ていく。
