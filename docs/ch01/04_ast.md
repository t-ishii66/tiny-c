...
`ast.h` でノードの型を定義し、`ast.c` でノードを作る関数（コンストラクタ）を書く。第1章で必要なのは4種類のノードだけだ。


## 1. なぜ「木」なのか

ソースコードは文字列で、コンパイラが扱うには不便だ。「`return` の後に書かれている式は何か」を答えるのに、文字列だと毎回切り出しと解釈をやることになる。`2 + 3 * 4` のような式になればますます辛い。

そこで、ソースコードを一度**メモリ上の構造化されたデータ**に変換しておく。「return 文ノードの子に式ノードがある。その式ノードは整数リテラルで、値は42」── 構造として保持しておけば、後の処理は単純なポインタ辿りになる。
...
文字列としての扱いは構文解析までで終わる。それ以降のすべての処理（型チェック、最適化、コード生成）はこの **木の上で** 行う。
...
案Bは確かに使わないフィールドが残るが、ノード1個あたり数十バイトの無駄に過ぎない。コンパイラが扱うノード数はせいぜい数千個程度で、メモリは問題にならない。

「`switch (node->kind)` で枝分かれする」というシンプルなパターンが、コンパイラ全体で一貫して使える。
...
このパターンは次の節のコード生成（`codegen.c`）でもまったく同じ形で出てくる。**木を再帰で辿る**── これがコンパイラの基本動作だ。
...
次の節（`05_codegen.md`）では、この木を辿って x86 アセンブリを出力するコード生成の中身を見ていく。
