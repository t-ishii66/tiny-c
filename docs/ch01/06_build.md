...
肉付けが起きるのは `lexer.l`（新しいトークン）、`parser.y`（新しい文法ルール）、`ast.h/ast.c`（新しいノード種別）、`codegen.c`（新しいコード生成パターン）の4ファイル。**コンパイラの成長はこの4つで起きる**。


## 4. Makefile を読む

> **読み流して大丈夫**: 以降の Makefile 解説は **雰囲気がわかれば十分**。1行ずつ覚える必要はない。実務でゼロから Makefile を書く機会は、最近ではそれほど多くない（多くのプロジェクトは CMake、Cargo、Bazel、npm、Go モジュールなど別のビルドツールを使う）。本書で `Makefile` を採用しているのも、依存関係を簡潔に表現できる教材として優れているという理由だ。本筋（コンパイラの中身）から脇道に逸れすぎないように、「こういう仕組みでビルドが回っている」と眺める程度で次に進んでよい。
...
合計 **約270行**。第2章以降はこの骨格に lexer / parser / AST / codegen の肉を足していくだけになる。
