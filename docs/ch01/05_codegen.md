...
AST がメモリ上にできた。コンパイラの最後の仕事は、この木から**アセンブリ**を出力することだ。

第1章で生成するアセンブリは、たった7行。
...
このアセンブリの一行一行が何を意味するのかを、最小限の知識から積み上げていく。アセンブリを書いたことがなくても、ここまで来た読者なら必ず理解できる。
...
x86-64 には汎用レジスタが16個ある。第1章で使うのはこの3つだけ：

| レジスタ | 役割 |
|---------|------|
| `%rax` | 戻り値や計算の主役。32ビット部分は `%eax` |
| `%rsp` | スタックポインタ。スタックの先端を指す |
| `%rbp` | ベースポインタ。関数のスタックフレームの起点 |
...
「Intel 構文だと逆向きで、`mov eax, 42` になる」と聞いたことがあるかもしれない。AT&T では `mov $42, %eax` のように「**動かす元 → 動かす先**」の順だ。慣れの問題なので、最初に違和感があっても気にしなくてよい。


## 4. 関数とスタック ── 値はどこに置くか

ここからが少しややこしい。「関数を呼ぶ」ってどういうことか、CPU の側から見てみよう。

関数を呼ぶには次のことをする必要がある。

1. **戻り先のアドレスを覚えておく** — 呼ばれた関数が終わったら、呼び出し元に戻らないといけない。
2. **ローカル変数の置き場所を確保する** — 関数の中で使う変数を置く場所が必要。
3. **計算の途中結果を保管する** — レジスタは数が限られるので、複雑な式では一時的にメモリに退避する場面が出る。

これらに使われるのが**スタック**だ。スタックはメモリの一領域で、関数呼び出しのたびに伸び、関数から戻ると縮む。
...
`%rsp` は**スタックの先端**を指すレジスタ。値を積む（`push`）と `%rsp` が下に動き、降ろす（`pop`）と上に戻る。
...
これを覚えておけばいい：「**`int` の戻り値は `%eax` にセットして `ret`**」。第1章のコード生成は、この事実を活かすだけだ。
...
`main` ラベルがないと、リンカが「`main` ってどこ？」となる。`.globl` がないと、リンカから見えない（同じファイルの中だけの私的な名前になる）。**`.globl main` + `main:`** がセットで「外部に公開された関数の入り口」を作る。
...
たった3行で「`return 42;` を return 文として正しく実装している」。式の評価という部分問題を `gen_expr` に委ね、自分は「戻る」処理を出すだけ。**役割分担**が綺麗だ。

`NODE_BLOCK` の場合：
中の文をリスト順に再帰的に処理するだけ。ブロック自体は何のコードも出さない。これは前のサブ章で見た「AST と再帰」のパターンそのものだ。
...
外部に公開するのは `codegen()` だけ。`gen_expr` や `gen_stmt` は `static` で隠してある。**API は最小限**にするのがいい習慣だ。
...
並びがほぼ AST の構造そのままだと気づくはず。

- `FUNC_DEF` ノードからプロローグが出る
- `RETURN` ノード配下の `INT_LIT` から `movl` が出る
- `RETURN` ノード自体からエピローグが出る

**「木の形」と「命令の順序」が対応している**。これは偶然ではない。AST を再帰で辿るとき、訪問順序が自然と命令の出力順序になる。コンパイラがシンプルに書ける根本的な理由だ。

第2章で `2 + 3 * 4` のような複雑な式を扱うと、この「木の形 ≒ 命令の順序」の関係がもっと面白くなる。式の評価順序が、実は AST の形によって完全に決まっているのだ。


## 10. まとめ

- アセンブリは **機械語の人間向け表現**。1対1で機械語に対応する。
- レジスタは **CPU 内部の高速な作業箱**。`%eax`（戻り値）、`%rsp`（スタック先端）、`%rbp`（フレーム起点）の3つを使い分ける。
- AT&T 構文は **`mov src, dst`** の順、レジスタは `%`、即値は `$`、命令名末尾にサイズ接尾辞。
- 関数の出入りには**プロローグ**（`pushq %rbp` + `movq %rsp, %rbp`）と**エピローグ**（`leave` + `ret`）が必要。
- C の戻り値の約束は「**`int` は `%eax` にセット**」。
- `codegen.c` は3つの関数で構成される：エントリポイント `codegen`、文の処理 `gen_stmt`、式の処理 `gen_expr`。
- 木を再帰で辿るパターンは、`print_ast` で見たものと**完全に同じ**。コンパイラの主要な動作は再帰だ。


## 次へ

これでコンパイラの中身（lexer、parser、AST、codegen）はすべて揃った。最後のサブ章（06_build.md）では、これらを統合する `main.c` と、ビルドの全工程を記述する `Makefile` を見て、実際にコンパイラを完成させる。
