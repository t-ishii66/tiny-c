...
ch01 と同じ流れだ。`tinyc` に `.c` ファイルを渡すとアセンブリが出る。
...
ch01 のときは「即値を `%eax` に入れて返すだけ」の3行だった。それが、`pushq` `popq` `imull` `addl` を含む11行に増えている。これが「式を評価するアセンブリ」の典型形だ。
...
`+` のノードの右の子が `*` のノードになっている。つまり「最初に `3*4` を計算し、その結果を `2` に足す」という構造が、木の形で表現されている。

**この木の形を作るのは parser.y の仕事**だ。次のサブ章で見ていく。
...
ch01 と違って flex、bison、AST、x86 の基礎は再説明しない。ch01 の道具をそのまま使う。だから章としてはぐっと短い。

## 4. 次へ

サブ章 01（`01_precedence.md`）では、`2 + 3 * 4` がなぜ `2 + (3*4)` になるのか、その仕組みを文法のレベルで見る。`parser.y` の書き方ひとつで、優先順位は自然に表現できる ── これが BNF 文法の美しいところだ。
