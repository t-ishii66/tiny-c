...
stmts
    : /* empty */       { $$ = NULL; }
    | stmt stmts        { $$ = new_node_list($1, $2); }       /* 右再帰に変更 */
    ;
...
ch02 と同じ。これで2章連続で main/Makefile を変えていない。**インターフェースが安定している証拠**。
...
時刻 1 (subq $8 後):         %rsp = %rbp - 8    スタック: [旧rbp][?]
時刻 2 (mov x = 1 後):       %rsp = %rbp - 8    スタック: [旧rbp][1]
時刻 3 (subq $8 後):         %rsp = %rbp - 16   スタック: [旧rbp][1][?]
時刻 4 (mov y = 2 後):       %rsp = %rbp - 16   スタック: [旧rbp][1][2]
時刻 5 (push rax = y 退避):  %rsp = %rbp - 24   スタック: [旧rbp][1][2][2]
時刻 6 (pop rcx 後):         %rsp = %rbp - 16   スタック: [旧rbp][1][2]
時刻 7 (leave 後):           %rsp = (元の値)    スタック: 復元
```

注目すべきは時刻 5。式の評価で `pushq %rax` が動くと、`%rsp` は `-24` まで下がる。**でも `-8(%rbp)` と `-16(%rbp)` は `%rbp` 基準のアドレスなので、変わらず x と y を指している**。`%rsp` が動こうが動くまいが、変数アクセスは安定だ。
...
- 変数の宣言・参照・代入。
- スタックフレームに `subq $8, %rsp` で領域を確保。
- 名前 → オフセットを覚える **シンボルテーブル**。
- lvalue / rvalue という見方の入り口。

第4章では、ここに **分岐 (if/else)** と **ループ (while)** を加える。比較演算子 (`< <= > >= == !=`) と論理否定 `!` も同時に。codegen の新しい道具は **ラベル** と **条件ジャンプ**。「CPU は条件ジャンプしか知らない」── どんなプログラムも、CPU レベルで見れば結局はジャンプの組み合わせだ、ということを次の章で実感する。

## まとめ

- 変数 = 名前 + スタック上のアドレス。コンパイラは **シンボルテーブル** で対応を覚える。
- アドレスは `%rbp` 基準の負のオフセット。`%rbp` は関数中ずっと動かない基準点。
- `subq $8, %rsp` でスタック領域を確保。lazy に変数1つにつき1回。
- 代入式の左辺は **lvalue**。ch03 では `IDENT` のみ許す。
- main.c / Makefile は変えない。**フロントとバックを分離した設計** の効果。
